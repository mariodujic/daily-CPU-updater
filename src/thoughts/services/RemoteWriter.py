...
class RemoteWriter(Write):
    ...
    def __get_collection(self, locale: str):
        return self.remote_database.get_firestore_client().collection(self.__get_firebase_environment_reference(locale))

    def __get_reference(self, locale: str):
        print(self.__get_firebase_environment_reference(locale))
        return self.remote_database \
            .get_database_client() \
            .reference(self.__get_firebase_environment_reference(locale)) \
            .child("latest")

    def __get_firebase_environment_reference(self, locale: str):
        if locale == "en":
